...
def compute_dlat_dlon(nc_file, save_file = False):
    ...
    dictionary = {'lon': lon_array.reshape(-1),
                  ...
                  'd_lon': d_phi.reshape(-1)}

    df = pd.DataFrame.from_dict(dictionary)
    if save_file:
        df.to_csv( 'lat_lon_dlat_dlon.csv' )
    return
...
def compute_area_one_cell(lat = 30, lon = -15):
    era_AREA = area_grid_cell(lat, 0.375, 0.375)
    coord_info = pd.read_csv('/home/hanna/Desktop/examples_master_thesis/lat_lon_dlat_dlon.csv')

    c_lat = coord_info.latitude.values
    c_lon = coord_info.longitude.values

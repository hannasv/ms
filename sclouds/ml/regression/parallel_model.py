...
if __name__ == '__main__':
    ...
    tr_data_sel =  train_dataset.sel(longitude = slice(min(longitudes), max(longitudes)))
    te_data_sel =  test_dataset.sel(longitude = slice(min(longitudes), max(longitudes)))

    p = Process(target=config_model, args=(start, stop, test_start,
                           test_stop, tr_data_sel, te_data_sel, order, transform,
                            sigmoid, latitudes, longitudes, type))
    ...
    #    print('Enters join .. ')
    p.join()
    print('passes p join before threads are finished')

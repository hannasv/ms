...
fig.suptitle('ERA5 (2014-01-01): ' + LONGNAME[var], fontsize = 14)
#plt.axis('off')
...
for i, ax in enumerate(axes.flatten()):
    ...
    # plot target
    vals    = np.flipud(test[var].values) # need to rotate data in ERA5
    vals[vals>1]=np.nan
...
plt.savefig(path_python_figures + 'ERA5_timelapse_cloud_cover_24hrs_from_{}.png'.format(str(test.time.values)[:10]))
